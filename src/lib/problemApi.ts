...
const getProblemOwner = (problem: Pick<BackendProblemListItem, "author" | "creator">) =>
  problem.author ?? problem.creator ?? null;
...
const mapProblemDetail = (problem: BackendProblemDetail): BackendProblem => {
  ...
  const owner = getProblemOwner(problem);
...
export async function updateProblem(
  ...
  const response = await fetch(`${API_V1_BASE_URL}/problems/${id}`, {
    ...
    body: JSON.stringify({
      ...
      test_cases: payload.testCases?.map((testCase, index) => ({
        input: testCase.input,
        output: testCase.expectedOutput,
        is_hidden: testCase.isHidden ?? index !== 0,
      })),
...
export async function getAssignments(token?: string): Promise<BackendAssignment[]> {
  const response = await fetch(`${API_V1_BASE_URL}/assignments`, {
    cache: "no-store",
    headers: getAuthHeaders(token),
  });
...
export async function getSubmissionsByUser(
  ...
  const response = await fetch(`${API_V1_BASE_URL}/users/${encodeURIComponent(username)}/submissions`, {
    cache: "no-store",
    headers: getAuthHeaders(token),
  });
  ...
  return extractList(await response.json()).map((submission) =>
    mapSubmission(submission as BackendSubmissionApi)
  );
}
...
export async function getProblemResults(id: number, token?: string): Promise<ProblemResultStats> {
  const assignments = (await getAssignments(token)).filter((assignment) => assignment.questionId === id);
