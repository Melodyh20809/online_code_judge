...
export default function ProblemPageSoluction({ problemId }: { problemId: string }) {
  ...
  useEffect(() => {
    const fetchAllSolutions = async () => {
      setIsLoading(true);
      try {
        // 由於後端 API 規範中沒有明確的解決方案端點
        // 可以透過查詢該題目的所有提交來做為臨時方案
        // 或等待後端提供相應的解決方案 API
        if (!session?.user?.username || !session.user.accessToken) {
          setAllSolutions([]);
          setFilteredAllSolutions([]);
          return;
        }
        await axios.get(`${API_BASE_URL}/api/v1/users/${session.user.username}/submissions`, {
          headers: { Authorization: `Bearer ${session.user.accessToken}` }
        });

        // TODO: 當後端提供解決方案 API 時，更新此邏輯
        setAllSolutions([]);
        setFilteredAllSolutions([]);
      } catch (error) {
        if (axios.isAxiosError(error) && error.response) {
          toast.error(error.response.data.message || "取得解決方案失敗");
          console.log("取得解決方案失敗: ", error.response.data.message);
        } else {
          toast.error("取得解決方案出錯");
          console.log("取得解決方案出錯: ", error);
        }
      } finally {
        setIsLoading(false);
      }
    ...
    fetchAllSolutions()
  }, [problemId, session?.user?.username, session?.user?.accessToken]);
